...
uint32_t adc0_voltage_buffer[get_size_buffer(voltage_sequencer)];   //буфер напряжения якоря
uint32_t adc0_current0_buffer[get_size_buffer(current0_sequencer)]; //буфер тока якоря
uint32_t adc0_current1_buffer[get_size_buffer(current1_sequencer)]; //буфер тока возбуждения
...
inline void ADC_init()
{
    ...
    GPIOPinConfigure(GPIO_PCTL_PE1_AIN2);                                                                    // конфигурация как AFIO
